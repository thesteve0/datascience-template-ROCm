...
- Backend specified in `customizations.jetbrains.backend: "PY"`
- `.idea/` directory auto-generated by JetBrains (not in repository)
- **Manual configuration required**:
  - Python interpreter: `/workspaces/PROJECT_NAME/.venv/bin/python`
  - Source roots: Mark `src/` as Sources, `tests/` as Test Sources
  - Excludes: Mark `.venv/`, `models/`, `datasets/`, `.cache/` as Excluded

**Python Interpreter Limitation**: JetBrains does not support automatic interpreter configuration in devcontainers ([IJPL-174150](https://youtrack.jetbrains.com/issue/IJPL-174150)). Users must manually configure via Settings UI.
